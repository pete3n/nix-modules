  ...
  hyprlidmon =
    pkgs.writeShellScriptBin "hyprlidmon" # sh
      ''
				...
				have_external() {
					# external = any enabled monitor whose name != INT_DISP
				...
				handle_lidClosed() {
					...
					${lib.concatStringsSep "\n" (map (rule: let
						...
						condExpr = if conds == [ ] then
							"true"
						else
							lib.concatStringsSep " && " (map (cond:
    ...
    rules = lib.mkOption {
      type = lib.types.listOf (
        lib.types.submodule {
          options = {
            cond = lib.mkOption {
              type = lib.types.listOf (lib.types.enum [ "extPower" "extDisplay" ]);
              default = [ ];
            };
